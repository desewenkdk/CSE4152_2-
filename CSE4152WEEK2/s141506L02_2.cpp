...
using namespace std;
using namespace cv;
void medianBlurFiltering_opencvCommand(int value, void *userData);//flag에 따라 cv함수, user-defined함수 중 선택해서 실행해야 하는데 그걸 콜백에서 처리? 아니면 서로 다른 콜백?
...
void medianBlurFiltering_opencvCommand(int value, void *userData) {
	...
	start_time = clock();
	medianBlur(input_im, output_dst, value);
...
int main(int argc, char *argv[])
{
	...
	createTrackbar("Median Filtering Window", window_name, &start_value, max_value, medianBlurFiltering_opencvCommand, static_cast<void*>(&window_name));
